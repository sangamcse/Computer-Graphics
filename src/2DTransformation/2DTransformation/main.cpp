...
//  2DTransformation
//
//  Created by Sangam Kumar on 08/11/17.
//  Copyright © 2017 Sangam Kumar. All rights reserved.
//
...
arr* mult(arr* x, arr* y){
    int i,j,k;
    int r1 = x->rows.size();
    int c1 = x->rows[0].size(), r2 = y->rows.size(), c2 = y->rows[0].size();
...
void translate(int tx, int ty, int flush){
    float xt = tx;
    float yt = ty;
    float tsup[] = {1,0,xt,0,1,yt,0,0,1};
...
int main(int argc, char** argv){
    ...
    glutCreateWindow("Many Amaze Very GL");
